// -README should cite sources for information (i.e. Yelp used for business info)

// AIzaSyCSvlguX2axX4hGDPmlp2c99BeAX4sjtwE --GoogleMaps API Key-- Browserkey1
// Yelp api key: lD2MMgGYV897NrXUeO0yEw Yelp consumer secret: zdScmshgUyGbQBftQ_nwAu9dAEU token: DhdwKqheVW1g5putE65ij4TRdq8XMkE0 token secret: tJkO7O03alkN9cIWb9r-qJZm1z4
//api keys delete?, Readme, change name of javascript files
...
function initMap() {
	...
	if (typeof(google) == 'undefined') {
		...
		var SantaBarbara = {lat: 34.42001523545522, lng: -119.69982250000004};
	...
	if (request.placeId != undefined) {
		service.getDetails(request, function(place, status) {
			if (status === google.maps.places.PlacesServiceStatus.OK) {
				...
				Model.details.push({idnum : index});
				index++;
				if (index <= placeId.length){
					GooglePlacesRequest(index);
				};
				...
				if (ErrorCounter < 3) {
					setTimeout(function(){GooglePlacesRequest(index);}, delay);
				}else {
					Model.details.push({idnum : index});
...
GooglePlacesGetPhotoUrls = function() {
	Model.details.forEach(function(detailobj, i){
		if (detailobj.hasOwnProperty('photos') && detailobj.photos.length > 0) {
			var src = detailobj.photos[0].getUrl({'maxWidth': 200,'maxHeight':100});
			Model.imagesrcs.push(src);
		}else if (Model.businesses[i].hasOwnProperty('photos') && Model.businesses[i].photos.length > 0){
			var src = Model.businesses[i].photos[0].getUrl({'maxWidth': 200,'maxHeight':100});
			Model.imagesrcs.push(src);
		}else {
	...
	success: function(results) {
		results.businesses.forEach(function(result) {
			...
			Model.details.forEach(function(detail) {
				...
				var resultInReviews = Model.reviews.filter(function(e){return e.name == result.name;}).length;
				var resultInReviewsId = Model.reviews.filter(function(e){return e.id == result.id;}).length;
				if (detail.hasOwnProperty('international_phone_number')) {
				...
				//If Google Places fails due to an error, compare results with less accurate location search.
					...
					//full address) then compare strictly by name.
						...
						var resultInBusiness = Model.businesses.filter(function(e){return e.name == result.name;}).length;
						if (resultInBusiness == 1) {
							addToReview(result, detail.idnum);
						//If Yelp's data for addresses do not match google's and yelp reports 
						//multiple businesses with the same phone number - use best match
						}else {
							var nameInReviews = Model.reviews.filter(function(e){return $.inArray(businessName, reviewid5);});
							if (nameInReviews > -1) {
								addToReview(result, detail.idnum);
							}
						}
